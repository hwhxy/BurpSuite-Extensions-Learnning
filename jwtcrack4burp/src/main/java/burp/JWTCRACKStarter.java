...
public class JWTCRACKStarter implements Runnable {

    @Override
    public void run() {
        try {
            String command = String.format("%s \"%s\" -m blasting -s %s --kf %s", Config.getPythonName(), Config.getJWTCRACKPath(), Config.getjwt(), Config.GetFuzzPath());
            ...
            if (osType == Util.OS_WIN) {
                ...
                */
                ...
                String optionCommand = Config.getJWTCRACKOptionsCommand();
                //将参数数中的"转译为\"
                optionCommand = optionCommand.replace("\"", "\\\"");
                command = String.format("%s \\\"%s\\\" -m blasting -s %s --kf %s", Config.getPythonName(), Config.getJWTCRACKPath(), Config.getjwt(), Config.GetFuzzPath());
                cmds.add("osascript");
                cmds.add("-e");
...
//        new Thread(new JWTCRACKStarter()).start();
